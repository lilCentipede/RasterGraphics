...
Image* Core::createImage(std::string& _name) {
    ...
    if (magicNumber == "P1") {
        ...
        in.close();
        in.open(_name,std::ios::in | std::ios::binary);
        unsigned int width;
        unsigned int height;
        in >> magicNumber >> width >> height;
        image = new PBM("P4", width, height, _name);
        image->readPixelsBinary(in);
        return image;
    }
        ...
        unsigned int height;
        unsigned int maxColor;
        in >> magicNumber >> width >> height >> maxColor;
        image = new PGM("P5", width, height, _name, maxColor);
        image->readPixelsBinary(in);
        return image;
    }
        ...
        unsigned int maxColor;
        in >>magicNumber >> width >> height >> maxColor;
        image = new PPM("P6", width, height, _name, maxColor);
        image->readPixelsBinary(in);
        return image;
    }
